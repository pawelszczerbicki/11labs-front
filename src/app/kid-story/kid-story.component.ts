...
export class KidStoryComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http.get<Kid[]>("/kid").pipe(
      tap(k => this.kid = k[0]),
      switchMap(() => this.http.get<Story>("/story/" + this.route.snapshot.params['id']).pipe(tap(s => this.story = s))),
  ...
  generateImage = () => this.http.post<any>("/story/image", {text: this.generatedStory?.story}).pipe(tap(s => this.generatedImage = s.url))
  ...
  choose = (o: string) => {
    ...
    this.generateText(o)
      .pipe(tap(s => this.animateText(s.story)),
        switchMap(() => forkJoin([
          this.generateAudio(),
          this.generateImage()
        ])),
  ...
  generateText = (selected?: string) => this.http.post<StoryTextResponse>("/story/text", {
    step: ++this.step,
    name: this.kid?.name,
    age: this.kid?.age,
  ...
  }).pipe(tap(s => this.generatedStory = s), tap(s => this.storyHistory.push(s.story)));
